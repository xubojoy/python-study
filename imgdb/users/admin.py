...
class UserModelFilter(admin.SimpleListFilter):
    ...
    def lookups(self, request, model_admin):
        user_types = set(
            [user.user_type for user in model_admin.model.objects.all()])
...
class UserAdmin(BaseUserAdmin):
    fieldsets = (
        (None, {
            'fields': ('username', 'real_name', 'email', 'password',)
        }),
        (_('Permissions'), {
    ...
    activate_users.short_description = _(
